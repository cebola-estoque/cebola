...
module.exports = function (cebola, options) {
   ...
   */
  operationCtrl.registerEntry = function (shipment, product, quantity, operationData) {
   ...
   * @return {Bluebird -> ProductOperation}
   */
  operationCtrl.registerExit = function (shipment, product, quantity, operationData) {
    if (!quantity || quantity >= 0) {
      return Bluebird.reject(new errors.InvalidOption('quantity', 'required'));
    }
  ...
  operationCtrl.registerLoss = function (product, quantity, operationData) {
    ...
    .then((available) => {
      ...
      return operationCtrl.registerExit(null, product, quantity, operationData);
    });
