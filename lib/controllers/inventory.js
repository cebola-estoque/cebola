...
module.exports = function (cebola, options) {
   ...
   *         - keepRecords
   *         - loadFullProductModel
   *         - loadFullProductSourceShipment
  ...
  inventoryCtrl.summary = function (query, filter, sort, options) {
    ...
    if (options.loadFullProductModel) {
      /**
       * Load full product.model
       */
      aggregation.lookup({
        from: 'productmodels',
        localField: 'product.model._id',
        foreignField: '_id',
        as: 'product.__models',
      });
    }
    ...
    if (options.loadFullProductSourceShipment) {
      /**
       * Load full product.sourceShipment
       */
      aggregation.lookup({
        from: 'shipments',
        localField: 'product.sourceShipment._id',
        foreignField: '_id',
        as: 'product.__sourceShipments',
      });
    }

    var FINAL_PROJECTION = {
      _id: 0,
      exited: 1,
      entered: 1,
      allocatedForExit: 1,
    ...
    if (options.loadFullProductModel) {
      FINAL_PROJECTION.product.model = {
        "$arrayElemAt": ['$product.__models', 0]
      };
    }
    
    if (options.loadFullProductSourceShipment) {
      FINAL_PROJECTION.product.sourceShipment = {
        "$arrayElemAt": ['$product.__sourceShipments', 0]
      }
    }

    aggregation.project(FINAL_PROJECTION);
  ...
  inventoryCtrl.shipmentSummary = function (shipment, query, filter, sort, options) {
    ...
    shipmentId = util.normalizeObjectId(shipmentId);
  ...
  inventoryCtrl.productSummary = function (product, query, filter, sort, options) {
    ...
    query['product.model._id'] = util.normalizeObjectId(product.model._id);
    // ensure the product expiry is at the right format
    query['product.expiry'] = util.normalizeExpiryDate(product.expiry);
    query['product.measureUnit'] = util.normalizeMeasureUnit(product.measureUnit);
    query['product.sourceShipment._id'] = util.normalizeObjectId(product.sourceShipment._id);
