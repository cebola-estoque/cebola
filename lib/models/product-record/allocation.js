...
module.exports = function (ProductRecord, conn, cebola, options) {
    ...
    allocatedQuantity: {
      ...
      validate: {
        validator: function (v) {
          ...
          switch (type) {
            case CONSTANTS.ALLOCATION_TYPES.ENTRY:
              return v > 0;
              break;
            case CONSTANTS.ALLOCATION_TYPES.EXIT:
              return v < 0;
              break;
            default:
              return false;
  ...
  /**
   * Let allocation schema track modifications
   * of `allocatedQuantity and scheduledFor`
   * @type {Array}
   */
  allocationSchema.plugin(mongooseHistory, {
    properties: [
      'allocatedQuantity',
      'scheduledFor',
    ]
